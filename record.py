...
for gesture in GESTURES:
    ...
    remaining = PER_PERSON - existing
    if remaining <= 0:
        print(f"Geste {gesture}: bereits {existing}/{PER_PERSON} vorhanden, übersprungen.")
        continue

    print(f"\n{'='*40}")
    print(f"  Geste: {gesture}  --  {remaining} Aufnahmen (bereits {existing}/{PER_PERSON})")
    print(f"{'='*40}")
